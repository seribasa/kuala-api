...
import { handleRefreshToken } from "../../../kuala/handlers/auth/refresh-token.ts";
...
Deno.test("handleRefreshToken - should use fallback URL when AUTH_BASE_URL is not set", async () => {
	...
	try {
		const mockContext = createMockContext({
			refresh_token: "test_refresh_token",
		}, "https://kuala-api.example.com/auth/refresh-token");
...
Deno.test("handleRefreshToken - should handle Supabase error without error_description", async () => {
...
Deno.test("handleRefreshToken - should handle Supabase error with no message fields", async () => {
...
Deno.test("handleRefreshToken - should handle response with missing status fallback", async () => {
