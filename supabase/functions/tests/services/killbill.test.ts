...
describe("KillBillService", () => {
	...
	describe("getOrCreateAccount", () => {
		it("returns existing account", async () => {
			...
			const res = await killBillService.getOrCreateAccount(
				"u1",
				"test@test.com",
			);
				...
				returnsNext([
					...
					Promise.resolve(
						new Response(null, {
							status: 201,
							headers: new Headers({
								"Location": "http://loc/acc-2",
							}),
						}),
					), // POST create
					Promise.resolve(
						new Response(JSON.stringify({ accountId: "acc-2" }), {
							status: 200,
						}),
					), // GET location
				]),
	...
	describe("getAccountByExternalKey", () => {
		...
		it("throws if create failure", async () => {
			...
			await assertRejects(
				() => killBillService.getAccountByExternalKey("u1"),
				Error,
				"Failed to get account: 500",
			);
		});
	});
	...
	describe("createAccount", () => {
		it("throws if create fails", async () => {
			...
			await assertRejects(
				() => killBillService.createAccount("u1", "test@t.com"),
				Error,
				"Failed to create Kill Bill account: 400",
			);
		});
		...
		it("throws if no location header", async () => {
			...
			await assertRejects(
				() => killBillService.createAccount("u1", "test@t.com"),
				Error,
				"Failed to get account location",
			);
		});
			...
			fetchStub = stub(
				...
				returnsNext([
					Promise.resolve(
						new Response(null, {
							status: 201,
							headers: new Headers({ "Location": "http://loc" }),
						}),
					),
					Promise.resolve(new Response(null, { status: 500 })),
				]),
	...
	describe("getSubscriptionById", () => {
		...
		it("404", async () => {
			...
			await assertRejects(
				() => killBillService.getSubscriptionById("sub-1"),
				Error,
		...
		it("other error", async () => {
			...
			await assertRejects(
				() => killBillService.getSubscriptionById("sub-1"),
				Error,
				"Failed to get subscription: 500",
			);
		});
	});
	...
	describe("getSubscriptionByExternalId", () => {
		it("other error", async () => {
			...
			await assertRejects(
				() => killBillService.getSubscriptionByExternalId("sub-1"),
				Error,
	...
	describe("verifySubscriptionOwnership", () => {
		it("returns true", async () => {
			...
			const req = await killBillService.verifySubscriptionOwnership(
				"s",
				"u",
			);
		...
		it("returns false on mismatch", async () => {
			...
			fetchStub = stub(
				...
				returnsNext([
					...
					Promise.resolve(
						new Response(
							JSON.stringify({ accountId: "acc-wrong" }),
							{ status: 200 },
						),
					),
				]),
	...
	describe("getActiveSubscription", () => {
		...
		it("returns null if not active", async () => {
			...
			fetchStub = stub(
				...
				returnsNext([
					Promise.resolve(
						new Response(JSON.stringify({ state: "C" }), {
							status: 200,
						}),
					),
				]),
	...
	describe("createSubscription", () => {
				...
				returnsNext([
					Promise.resolve(
						new Response(null, {
							status: 201,
							headers: new Headers({
								"Location": "http://loc/sub-1",
							}),
						}),
					),
				]),
			);
			const req = await killBillService.createSubscription(
				"ext",
				"acc",
				"plan",
			);
		...
		it("duplicate", async () => {
			...
			fetchStub = stub(
				...
				returnsNext([
					Promise.resolve(
						new Response("Duplicate entry", { status: 400 }),
					),
				]),
			);
			await assertRejects(
				() => killBillService.createSubscription("ext", "acc", "plan"),
				Error,
		...
		it("other error", async () => {
			...
			await assertRejects(
				() => killBillService.createSubscription("ext", "acc", "plan"),
				Error,
				"Failed to create subscription: 400 - err",
			);
		});
	});
	...
	describe("cancelSubscription", () => {
		...
		it("error", async () => {
			...
			await assertRejects(
				() => killBillService.cancelSubscription("sub-1"),
				Error,
				"Failed to cancel subscription: 400 - err",
			);
		});
	});
	...
	describe("uncancelSubscription", () => {
		...
		it("error", async () => {
			...
			await assertRejects(
				() => killBillService.uncancelSubscription("sub-1"),
				Error,
				"Failed to uncancel subscription: 400 - err",
			);
		});
	});
	...
	describe("voidInvoice", () => {
		...
		it("error", async () => {
			...
			await assertRejects(
				() => killBillService.voidInvoice("inv-1"),
				Error,
				"Failed to void invoice: 400 - err",
			);
		});
	});
	...
	describe("triggerInvoiceRun", () => {
				...
				returnsNext([
					Promise.resolve(
						new Response(null, {
							status: 201,
							headers: new Headers({
								"Location": "http://loc/inv-1",
							}),
						}),
					),
				]),
			);
			const res = await killBillService.triggerInvoiceRun(
				"acc-1",
				"target_date",
			);
		...
		it("error", async () => {
			...
			await assertRejects(
				() => killBillService.triggerInvoiceRun("acc-1", "target_date"),
				Error,
				"Failed to trigger invoice run: 400 - err",
			);
		});
	});
	...
	describe("getAccountInvoices", () => {
		...
		it("error", async () => {
			...
			await assertRejects(
				() => killBillService.getAccountInvoices("acc1"),
				Error,
				"Failed to get account invoices: 500",
			);
		});
	});
