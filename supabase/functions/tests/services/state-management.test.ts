...
describe("StateManagementService Transitions & Validation", () => {
    describe("isValidTransition", () => {
        it("should return true for valid transitions", () => {
            ...
            assertEquals(isValidTransition("account_ready", "creating_subscription"), true);
            assertEquals(isValidTransition("creating_subscription", "failed"), true);
        });

        it("should return false for invalid transitions", () => {
             assertEquals(isValidTransition("requested", "completed"), false);
             assertEquals(isValidTransition("completed", "account_ready"), false);
        });

        it("should allow any transition for unknown from_state", () => {
             assertEquals(isValidTransition("some_unknown_state", "requested"), true);
        });

        it("should return true if entityType is not subscription_request", () => {
             assertEquals(isValidTransition("completed", "requested", "other_entity"), true);
        });
    });
    ...
    describe("StateManagementService API", () => {
        ...
        beforeEach(() => {
            ...
            mockSupabase = {
                ...
                select: () => mockSupabase,
                eq: () => mockSupabase,
                maybeSingle: () => ({ data: { current_state: "requested" }, error: null }),
        ...
        it("transitionState - throws InvalidStateTransitionError if validation fails", async () => {
             ...
             await assertRejects(
                 () => service.transitionState("subscription_request", "id1", "completed"),
                 InvalidStateTransitionError
             );
        });

        it("transitionState - valid transition succeeds", async () => {
             const rpcStub = stub(mockSupabase, "rpc", returnsNext([{ data: "mock-id", error: null }]));
             const result = await service.transitionState("subscription_request", "id1", "account_ready");
        ...
        it("transitionState - throws error on RPC failure", async () => {
             // current state is "requested"
             const rpcStub = stub(mockSupabase, "rpc", returnsNext([{ data: null, error: new Error("RPC Error") }]));
             await assertRejects(
                 () => service.transitionState("subscription_request", "id1", "account_ready"),
                 Error,
        ...
        it("getCurrentState - succeeds", async () => {
             const maybeSingleStub = stub(mockSupabase, "maybeSingle", returnsNext([{ data: { current_state: "running" }, error: null }]));
        ...
        it("getCurrentState - throws on error", async () => {
             const maybeSingleStub = stub(mockSupabase, "maybeSingle", returnsNext([{ data: null, error: new Error("DB Err") }]));
             await assertRejects(() => service.getCurrentState("entity", "id1"));
             maybeSingleStub.restore();
        });

        it("getStateHistory - succeeds", async () => {
             const rpcStub = stub(mockSupabase, "rpc", returnsNext([{ data: [{ to_state: "ready" }], error: null }]));
        ...
        it("getStateHistory - throws on error", async () => {
             const rpcStub = stub(mockSupabase, "rpc", returnsNext([{ data: null, error: new Error("DB Err") }]));
        ...
        it("isInAnyState - returns boolean", async () => {
             const maybeSingleStub = stub(mockSupabase, "maybeSingle", returnsNext([{ data: { current_state: "running" }, error: null }]));
             const res = await service.isInAnyState("entity", "id1", ["running", "failed"]);
             assertEquals(res, true);
             maybeSingleStub.restore();
        });

        it("getEntitiesInState - returns entities", async () => {
             mockSupabase.eq = () => ({ eq: () => Promise.resolve({ data: [{id: 1}], error: null }) });
             const res = await service.getEntitiesInState("entity", "state1");
             assertEquals(res.length, 1);
        });

        it("getEntitiesInState - throws on error", async () => {
             mockSupabase.eq = () => ({ eq: () => Promise.resolve({ data: null, error: new Error("DB ERR") }) });
             await assertRejects(() => service.getEntitiesInState("entity", "state1"));
        });

        it("getStateStatistics - computes stats", async () => {
             const lteStub = stub(mockSupabase, "lte", returnsNext([{ data: [{to_state: "A"}, {to_state: "A"}, {to_state: "B"}], error: null }]));
        ...
        it("getStateStatistics - throws on error", async () => {
             ...
             await assertRejects(() => service.getStateStatistics("entity", "2020", "2021"));
        });

        it("getEntitiesByMetadata - returns data", async () => {
             const containsStub = stub(mockSupabase, "contains", returnsNext([{ data: [{id: 1}], error: null }]));
             const res = await service.getEntitiesByMetadata("entity", "userId", "user1");
             assertEquals(res.length, 1);
             containsStub.restore();
        });

        it("getEntitiesByMetadata - throws on error", async () => {
             const containsStub = stub(mockSupabase, "contains", returnsNext([{ data: null, error: new Error("err") }]));
             await assertRejects(() => service.getEntitiesByMetadata("entity", "userId", "user1"));
             containsStub.restore();
        });

        it("bulkTransition - returns array of transitionIds and gracefully handles errors for some", async () => {
             // Current state is requested.
             const rpcStub = stub(mockSupabase, "rpc", returnsNext([
                 { data: "t1", error: null }, 
        ...
        it("waitForState - should resolve when state reached", async () => {
            const maybeSingleStub = stub(mockSupabase, "maybeSingle", returnsNext([{ data: { current_state: "target" }, error: null }]));
            const res = await service.waitForState("entity", "id", "target", 1000, 10);
            assertEquals(res?.current_state, "target");
            maybeSingleStub.restore();
        });

        it("waitForState - should reject on timeout", async () => {
            const maybeSingleStub = stub(mockSupabase, "maybeSingle", returnsNext([{ data: { current_state: "other" }, error: null }]));
            await assertRejects(() => service.waitForState("entity", "id", "target", 10, 1));
            maybeSingleStub.restore();
        });

        it("waitForState - should reject on failed state", async () => {
            const maybeSingleStub = stub(mockSupabase, "maybeSingle", returnsNext([{ data: { current_state: "failed" }, error: null }]));
            await assertRejects(() => service.waitForState("entity", "id", "target", 100, 10));
            maybeSingleStub.restore();
        });

        it("waitForState - should reject on cancelled state", async () => {
            const maybeSingleStub = stub(mockSupabase, "maybeSingle", returnsNext([{ data: { current_state: "cancelled" }, error: null }]));
            await assertRejects(() => service.waitForState("entity", "id", "target", 100, 10));
            maybeSingleStub.restore();
        });
    });
