...
describe("subscriptionStateManager Transitions", () => {
	...
	it("transitionToCreatingSubscription", async () => {
		...
		assertSpyCalls(transitionStateStub, 1);
		assertEquals(
			transitionStateStub.calls[0].args[2],
	...
	it("transitionToCompleted", async () => {
		const releaseLockStub = stub(
			subscriptionStateManager,
			"releaseUserLockByCorrelation",
			returnsNext([Promise.resolve(true)]),
		);
	...
	it("transitionToCompleted - handles lock release error", async () => {
		const releaseLockStub = stub(
			subscriptionStateManager,
			"releaseUserLockByCorrelation",
			returnsNext([Promise.reject(new Error("Fail"))]),
		);
		// Should not throw
		await subscriptionStateManager.transitionToCompleted("corr", {});
		assertSpyCalls(transitionStateStub, 1);
	...
	it("transitionToFailed", async () => {
		...
		assertEquals(
			transitionStateStub.calls[0].args[3].errorDetails.message,
			"err message",
		);
	});
...
describe("subscriptionStateManager Queries", () => {
	...
	it("hasPendingSubscriptionRequest - true", async () => {
		...
		const res = await subscriptionStateManager
			.hasPendingSubscriptionRequest("u1");
...
describe("subscriptionStateManager Locks", () => {
	...
	it("acquireUserLock - error", async () => {
		...
		await assertRejects(() =>
			subscriptionStateManager.acquireUserLock("u", "c")
		);
	});
	...
	it("releaseUserLock - error", async () => {
		...
		await assertRejects(() =>
			subscriptionStateManager.releaseUserLock("u")
		);
	});

	it("releaseUserLockByCorrelation", async () => {
		const res = await subscriptionStateManager.releaseUserLockByCorrelation(
			"c",
		);
		assertEquals(res, true);
	});
	...
	it("releaseUserLockByCorrelation - error", async () => {
		...
		await assertRejects(() =>
			subscriptionStateManager.releaseUserLockByCorrelation("c")
		);
	});
	...
	it("getActiveRequest - error", async () => {
		...
		await assertRejects(() =>
			subscriptionStateManager.getActiveRequest("u")
		);
	});
});
